...
export default function CadastroEmpresa() {
  ...
  const senhaForte = validations.senhaMin && validations.senhaMaiuscula && validations.senhaNumero;
  ...
  const strengthClass = ["", styles.weak, styles.medium, styles.strong, styles.veryStrong][strength];

  const validate = () => {
    const newErrors: { [key: string]: string } = {};
    if (!validations.nome) newErrors.nome = "Nome deve ter pelo menos 3 caracteres.";
    ...
    if (!senhaForte) newErrors.senha = "Senha deve ter 8+ caracteres, 1 maiúscula e 1 número.";
    if (!validations.confirmSenha) newErrors.confirmSenha = "As senhas não coincidem.";
    return newErrors;
  };
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.loginContainer}>
        <div className={styles.left}>
          ...
          <h1>Conecte-se aos <br /><span>melhores talentos</span></h1>
          <p className={styles.tagline}>
            Cadastre sua empresa e publique vagas em minutos. Processo validado e seguro.
          </p>
        </div>
          ...
          {success? (
            <div className={styles.successAnimation}>
              ...
              <svg className={styles.checkmark} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 52 52">
                ...
                <path className={styles.checkmarkCheck} fill="none" d="M14 27l7 7 17-17" />
              </svg>
              <h2>Conta criada!</h2>
              <p>Enviamos o código de verificação para seu e-mail corporativo.</p>
            </div>
          ) : (
            <div className={styles.cardContent}>
              ...
              <div className={styles.validationBox}>
                <div className={`${styles.validationItem} ${validations.nome? styles.valid : ""}`}>
                  <span className={styles.checkIcon}>✓</span> Nome válido
                </div>
                <div className={`${styles.validationItem} ${validations.cnpj? styles.valid : ""}`}>
                  <span className={styles.checkIcon}>✓</span> CNPJ válido
                </div>
                <div className={`${styles.validationItem} ${senhaForte? styles.valid : ""}`}>
                  <span className={styles.checkIcon}>✓</span> Senha forte
                </div>
                <div className={`${styles.validationItem} ${validations.confirmSenha? styles.valid : ""}`}>
              ...
              <form onSubmit={handleSubmit} noValidate>
                <div className={styles.inputGroup}>
                  <input type="text" name="nome" placeholder="Nome da empresa" value={form.nome} onChange={handleChange}
                    className={`${styles.input} ${errors.nome? styles.error : ""}`} />
                  {errors.nome && <p className={styles.errorMessage}>{errors.nome}</p>}
                </div>

                <div className={styles.inputGroup}>
                  <input type="text" name="cnpj" placeholder="CNPJ (00.000.000/0000-00)" value={form.cnpj} onChange={handleChange}
                    className={`${styles.input} ${errors.cnpj? styles.error : ""}`} maxLength={18} />
                  {errors.cnpj && <p className={styles.errorMessage}>{errors.cnpj}</p>}
                </div>

                <div className={styles.inputGroup}>
                  <input type="text" name="telefone" placeholder="Telefone (11) 99999-9999" value={form.telefone} onChange={handleChange}
                    className={`${styles.input} ${errors.telefone? styles.error : ""}`} />
                  {errors.telefone && <p className={styles.errorMessage}>{errors.telefone}</p>}
                </div>

                <div className={styles.inputGroup}>
                  <input type="email" name="email" placeholder="Email empresarial" value={form.email} onChange={handleChange}
                    className={`${styles.input} ${errors.email? styles.error : ""}`} />
                  {errors.email && <p className={styles.errorMessage}>{errors.email}</p>}
                </div>

                <div className={styles.inputGroup}>
                  <input type="text" name="endereco" placeholder="Endereço completo" value={form.endereco} onChange={handleChange}
                    className={`${styles.input} ${errors.endereco? styles.error : ""}`} />
                  {errors.endereco && <p className={styles.errorMessage}>{errors.endereco}</p>}
                </div>

                <div className={styles.inputGroup}>
                  <div className={styles.senhaBox}>
                    <input type={showSenha? "text" : "password"} name="senha" placeholder="Senha" value={form.senha} onChange={handleChange}
                      className={`${styles.input} ${errors.senha? styles.error : ""}`} />
                    <button type="button" className={styles.toggleSenha} onClick={() => setShowSenha(!showSenha)}>
                  ...
                  {form.senha && (
                    <div className={styles.passwordStrength}>
                      <div className={styles.strengthBar}>
                        <div className={`${styles.strengthFill} ${strengthClass}`}></div>
                      </div>
                      <span>{strengthLabel}</span>
                    </div>
                  )}
                  {errors.senha && <p className={styles.errorMessage}>{errors.senha}</p>}
                </div>

                <div className={styles.inputGroup}>
                  <div className={styles.senhaBox}>
                    <input type={showConfirmSenha? "text" : "password"} name="confirmSenha" placeholder="Confirmar senha" value={form.confirmSenha} onChange={handleChange}
                      className={`${styles.input} ${errors.confirmSenha? styles.error : ""}`} />
                    <button type="button" className={styles.toggleSenha} onClick={() => setShowConfirmSenha(!showConfirmSenha)}>
                      {showConfirmSenha? <EyeOpenIcon /> : <EyeClosedIcon />}
                    </button>
                  </div>
                  {errors.confirmSenha && <p className={styles.errorMessage}>{errors.confirmSenha}</p>}
                </div>
              ...
              <div className={styles.footerActions}>
                ...
                <p className={styles.subLink}>
                  Já tem conta? <a onClick={() => navigate("/loginEmpresa")}>Voltar para login empresarial</a>
                </p>
              </div>
            </div>
          )}
